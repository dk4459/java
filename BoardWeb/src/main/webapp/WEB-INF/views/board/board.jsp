...
<form action="modifyForm.do">
	...
	<table class="table table-striped w-75 mt-5" align="center">
		...
		<tr>
			<th>이미지</th>
			<td colspan="8"><img src="images/${board.img }"></td>
		</tr>
		<tr>
		...
		<c:if test="${msg != null}">
			<tr>
				<td colspan="9" align="center"><span style="color: red;">${msg }</span>
				</td>
			</tr>
		</c:if>
	</table>
	...
	document
			...
			.addEventListener(
					...
					function(e) {
						...
						let bno = document
								.querySelector('input[name="boardNo"]').value;
						console.log(writer, logId)
						if (writer.trim() == logId.trim()) {
							location.href = "deleteControl.do?boardNo="
									+ bno
									+ "&searchCondition=${search}&keyword=${keyword}&page=${page}";
						} else {
							alert('권한을 확인하세요');
						}
					});
